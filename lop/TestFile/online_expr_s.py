...
def online_expr(params: {}):
    ...
    #Setting of NC data saving path
    nc_data_dir = params['data_dir'].rstrip('/') + '_NC/'  
    nc_data_file = nc_data_dir + os.path.basename(params['data_file']).split('/')[-1]
    
    # Create the NC directory (if it doesn't exist)
    os.makedirs(nc_data_dir, exist_ok=True)
    
    # Initialize the NC data storage array
    nc1_values = torch.zeros(num_tasks, dtype=torch.float)
    nc2_values = torch.zeros(num_tasks, dtype=torch.float)
    nc3_values = torch.zeros(num_tasks, dtype=torch.float)
    nc4_values = torch.zeros(num_tasks, dtype=torch.float)
    ...
    net = DeepFFNN(input_size=input_size, num_features=num_features, num_outputs=classes_per_task,
                   num_hidden_layers=num_hidden_layers)  ##w (784, 100, 10, 3)
    ...
    rank_measure_period = 10000  ##w Let it to be examples_per_task
    ...
    with open('data/mnist_', 'rb+') as f:
        ...
        if use_gpu == 1:
            x = x.to(dev)
            y = y.to(dev)
#    dataset: TensorDataset = TensorDataset(x, y)  ##w for compute NC1-4
    dataset = TensorDataset(x, y)  ##w for compute NC1-4
    ...
    for task_idx in (range(num_tasks)):
        ...
        nc1, nc2, nc3, nc4 = NC(model=net, data_loader=dataloader, num_classes=10)
        print("nc1", nc1, "nc2", nc2, "nc3", nc3, "nc4", nc4)
        
        # Save the NC pointer to an array
        nc1_values[task_idx] = nc1
        nc2_values[task_idx] = nc2  
        nc3_values[task_idx] = nc3
        nc4_values[task_idx] = nc4

        # Regularly save the NC data (at the same frequency as the master data)
        if task_idx % save_after_every_n_tasks == 0:
            nc_data = {
                ...
                'nc3': nc3_values.cpu(),
                'nc4': nc4_values.cpu(),
                'task_indices': torch.arange(num_tasks).cpu()
            }
            save_data(file=nc_data_file, data=nc_data)
