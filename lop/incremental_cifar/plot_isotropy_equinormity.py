...
JOINT_RESULTS_DIR = os.path.join(RESULT_DIR, "joint")
...
def read_continual_data():
    path = os.path.join(RESULT_DIR, "0.csv")
    df = pd.read_csv(path)
    return df["isotropy"].values, df["equinormity"].values, df["task_idx"].values

def read_joint_data():
    # 联合训练只写出“实际接受 task 数”对应的文件（NC4 gating 后可能不足 20，
    # 例如 ..._13_tasks.csv），不能用固定数字拼文件名，按通配符找实际生成的那份。
    matches = sorted(glob.glob(
        os.path.join(JOINT_RESULTS_DIR, "joint_model_per_task_accuracy_*_tasks.csv")))
    if not matches:
        raise FileNotFoundError(
            f"在 {JOINT_RESULTS_DIR} 下找不到 joint_model_per_task_accuracy_*_tasks.csv")
    path = matches[-1]  # 取最新一份
    df = pd.read_csv(path)
    return df["isotropy"].values, df["equinormity"].values, df["task_idx"].values

def read_independent_data():
    path = os.path.join(RESULT_DIR, "independent_final_results.csv")
    df = pd.read_csv(path)
    return df["isotropy"].values, df["equinormity"].values, df["task_idx"].values
...
# 实际 task 数（三份文件应一致，取最大 task_idx 决定；用于颜色条归一化）
n_tasks = int(np.concatenate([cont_tasks, joint_tasks, indep_tasks]).max()) + 1
...
norm = plt.Normalize(vmin=0, vmax=n_tasks - 1)
...
# task index 是整数，用整刻度
tick_step = max(1, int(np.ceil((n_tasks - 1) / 5)))
cbar_ticks = list(range(0, n_tasks, tick_step))
if n_tasks - 1 not in cbar_ticks:
    cbar_ticks.append(n_tasks - 1)
...
# 保存
save_png = os.path.join(OUTPUT_DIR, "isotropy_equinormity_3subplots_with_right_cbar.png")
plt.savefig(save_png, dpi=300, bbox_inches='tight')
plt.close()

print(f"绘图完成！已保存到：{save_png}")
