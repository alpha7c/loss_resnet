...
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '2'  # 强制只用GPU4，彻底杜绝设备漂移
...
#                      approximate_ranks_abs, ranks, dead_neurons, dev, tasks_permutations,plasticity_file, skip_threshold):
...
#         # ========== 新增：预评估及可塑性指标（开始） ==========
#     criterion = torch.nn.CrossEntropyLoss()   # 与 Backprop 中的 F.cross_entropy 一致
...
#         # 注意：原返回 5 个值，现在要返回 6 个（最后一个是 skip=True）
...
#真
    ...
    print("="*80)
        ...
        # 在循环外（例如刚开始定义 save_dir 之后）
    plasticity_file = os.path.join(save_dir, 'plasticity.csv')
    ...
    SKIP_THRESHOLD = 0.5   # 临时阈值，后续根据 loss_pre 打印调整
    ...
    for task_idx in range(max_possible_tasks):
        ...
        #     tasks_permutations
        # )
        iter_count, current_learner, _, _, task_reached_threshold, skip_this_task = train_single_task(
                current_learner, x_original, y_original, task_idx, params, save_dir, traj_save_dir,
                num_hidden_layers, input_size, examples_per_task, change_after,
                mini_batch_size, rank_measure_period, iter_count, accuracies,
                weight_mag_sum, effective_ranks, approximate_ranks,
                approximate_ranks_abs, ranks, dead_neurons, dev,
                tasks_permutations,
                plasticity_file=plasticity_file,
        ...
        if not skip_this_task:
            ...
            print(f"[Task {task_idx}] Skipped, no model saved and no forward test.")
